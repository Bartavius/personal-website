...
export default function EducationSection() {
            ...
            timeline: [
                {
                    ...
                    lines: [
                        "Won FinHacks 2025",
                        "Participated in HackBeanpot 2025"
                    ]
                },
                {
                    time: "Spring 2024",
    ...
    const [toggle, setToggle] = useState("Course Work");
    const selectedTab = tabs.find(tab => tab.name === toggle);
  ...
  return (
    <div className="education-section">
      ...
      <div className="degree-section">
        ...
        <div className="degree-description">
            <div className="degree-title">B.S. Computer Science with a Concentration in Software</div>
            <div className="degree-body"><b>GPA:</b> 3.97 / 4.00</div>
            <div className="degree-body"><b>Honors:</b> Dean's List Fall 2023, Spring 2024, Fall 2024</div>
        </div>
      </div>
    ...
    {tabs.map(tab => (
        <button key={tab.name} onClick={() => setToggle(tab.name)} className={toggle === tab.name ? "active" : ""}>
            {tab.name}
        </button>
    ))}
</div>
            {selectedTab && <TimelineTree tree={selectedTab.timeline} />}
        </div>
    </div>
  );
}
